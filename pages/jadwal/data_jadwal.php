...
// Query filter dasar
$query = "
    SELECT j.*, g.nama_guru, g.mapel, k.nama_kelas, k.tingkat,
           ta.tahun_ajaran, ta.semester
    FROM jadwal j
    LEFT JOIN guru g ON j.id_guru = g.id
    LEFT JOIN kelas k ON j.id_kelas = k.id
    LEFT JOIN tahun_ajaran ta ON j.tahun_ajaran_id = ta.id_tahun_ajaran
    WHERE 1=1
";

// Tambahkan kondisi filter
if ($filter_tahun != '') {
  $query .= " AND j.tahun_ajaran_id = '$filter_tahun'";
}
if ($filter_hari != '') {
    ...
    <tbody>
      ...
      if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
          echo "<tr>
                  <td>{$no}</td>
                  <td>{$row['tahun_ajaran']} - {$row['semester']}</td>
                  <td>{$row['hari']}</td>
